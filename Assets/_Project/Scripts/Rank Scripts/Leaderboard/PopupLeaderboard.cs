...
{
    public class PopupLeaderboard : Popup
    {
        ...
        public class ElementColor
        {
            ...
            public ElementColor(Color colorBackground, Color colorOverlay, Color colorBoder, Color colorHeader,
                Color colorText)
            {
        ...
        protected class Data
        {
            ...
            public DateTime LastTimeRefreshLeaderboard
            {
                ...
                {
                    DateTime.TryParse(
                        PlayerPrefs.GetString($"{LAST_TIME_FETCH_RANK_KEY}_{_key}",
                            DateTime.UtcNow.ToString(CultureInfo.InvariantCulture)),
                        out var result);
                    return result;
                }
                set => PlayerPrefs.SetString($"{LAST_TIME_FETCH_RANK_KEY}_{_key}",
                    value.ToString(CultureInfo.InvariantCulture));
            }

            public bool IsCanRefresh(int limitTime) =>
                (DateTime.UtcNow - LastTimeRefreshLeaderboard).TotalSeconds >= limitTime || firstTime;
        }
        ...
        protected virtual void Refresh(Data data)
        {
            ...
            btnNextPage.gameObject.SetActive(data.currentPage < data.pageCount &&
                                             !(data.players.Count < 100 && data.currentPage == data.pageCount - 1));
            ...
            InternalConfig[] internalConfigs)
        {
            ...
            for (int i = 0; i < internalConfigs.Length; i++)
            {
                ...
                sequense.Append(rankSlots[i].transform.TweenLocalScale(new Vector3(1.04f, 1.06f, 1), 0.15f)
                    .SetEase(Ease.OutQuad));
                sequense.Append(rankSlots[i].transform.TweenLocalScale(Vector3.one, 0.08f)
                    .SetEase(Ease.InQuad));
                sequense.Play();
        ...
        private async void FetchInternalConfig(List<PlayerLeaderboardEntry> entries,
            Func<List<PlayerLeaderboardEntry>, InternalConfig[], IEnumerator<float>> onCompleted)
        {
            ...
            Timing.RunCoroutine(onCompleted?.Invoke(entries, configs), Segment.RealtimeUpdate);
        }
        ...
        protected virtual void OnNextPageButtonClicked()
        {
            ...
            switch (currentTab)
            {
                case ELeaderboardTab.World:
                    ...
                    {
                        if (worldData.currentPage * CountInOnePage >= worldData.players.Count &&
                            worldData.players.Count > 0)
                        {
                            block.SetActive(true);
                            content.SetActive(false);
                ...
                case ELeaderboardTab.Country:
                    ...
                    {
                        if (countryData.currentPage * CountInOnePage >= countryData.players.Count &&
                            countryData.players.Count > 0)
                        {
                            block.SetActive(true);
                            content.SetActive(false);
        ...
        private void OnGetLeaderboardAroundUserWorldError(PlayFabError error)
        {
            ...
            popupNotification.Message(
                $"Retrieve your position in the failed world ranking!\nError code: {error.Error}");
        ...
        private void NextPageRequestCountryLeaderboardError(PlayFabError error)
        {
            btnNextPage.interactable = true;
        }
        ...
        private void OnGetLeaderboardAroundUserCountryError(PlayFabError error)
        {
            ...
            var popupNotification = GameObject.Find("PopupNotification").GetComponent<PopupNotification>();
            popupNotification.Message(
                $"Retrieve your position in the failed country ranking!\nError code: {error.Error}");
            ...
            AuthService.GetStatistic(LoginResultModel.playerId,
                ...
                userResult =>
                {
                    friendData.players.Add(new FriendData.Entry()
                    {
                        ...
                        sprite =
 FacebookManager.CreateSprite(FacebookManager.Instance.ProfilePicture, Vector2.one * 0.5f),
        ...
        private async void FetchFriendDataFb()
        {
            ...
            var friendIds =
 FacebookManager.Instance.FriendDatas.Map(_ => _.id).Filter(_ => !friendCacheAvatar.ContainsKey(_));
        ...
        private async void FetchFriendConfig(List<FriendData.Entry> entries, Func<List<FriendData.Entry>, InternalConfig[], IEnumerator<float>> onCompleted)
        {
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                if (entries[i].sprite == null)
                {
                    ...
                    {
                        var status =
 await FacebookManager.Instance.LoadTextureInternal(FacebookManager.Instance.FriendDatas.First(_ => _.id.Equals(fbId)).pictureUrl);
                        if (status)
                        {
                            entries[i].sprite =
        ...
        public void CreateOrUpdateAggregationLeaderboard()
        {
            ...
            void Call()
            {
                ...
                PlayFabAdminAPI.CreatePlayerStatisticDefinition(
                    new PlayFab.AdminModels.CreatePlayerStatisticDefinitionRequest()
                    {
                    ...
                    error =>
                    {
                        if (error.Error == PlayFabErrorCode.StatisticNameConflict)
                        {
                            PlayFabAdminAPI.UpdatePlayerStatisticDefinition(
                                new PlayFab.AdminModels.UpdatePlayerStatisticDefinitionRequest()
                                {
                                    StatisticName = $"{nameTableLeaderboard}_{c.code.ToString()}",
                                    AggregationMethod = PlayFab.AdminModels.StatisticAggregationMethod.Max,
                                    VersionChangeInterval = PlayFab.AdminModels.StatisticResetIntervalOption.Never
